...
function DHData() {
  ...
  useEffect(() => {
    const fetchDHData = async () => {
      try {
        const response = await fetch('http://localhost:8082/dh/profile', {
          method: 'GET',
           credentials: "include",
          headers: {
  ...
  if (loading) {
    return <div className="text-center mt-4">Loading department head data...</div>;
  }
  ...
  if (!dhData) {
    return <div className="text-center mt-4">No dh data available</div>;
  }
        ...
        <div className="card bg-light p-3 rounded shadow-sm" style={{
          ...
          <div className="row">
            ...
            <div className="col-6 col-sm-4">
              <p className="mb-2 fw-medium text-secondary">{dhData.fname || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{dhData.lname || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{dhData.username || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{dhData.sex || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
