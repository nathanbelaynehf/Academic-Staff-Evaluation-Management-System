...
function DeanData() {
  ...
  useEffect(() => {
    const fetchDeanData = async () => {
      try {
        const response = await fetch('http://localhost:8082/ad/profile', {
          method: 'GET',
           credentials: "include",
          headers: {
        ...
        <div className="card bg-light p-3 rounded shadow-sm" style={{
          ...
          <div className="row">
            ...
            <div className="col-6 col-sm-4">
              <p className="mb-2 fw-medium text-secondary">{deanData.fname || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{deanData.lname || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{deanData.username || 'N/A'}</p>
              <p className="mb-2 fw-medium text-secondary">{deanData.sex || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
