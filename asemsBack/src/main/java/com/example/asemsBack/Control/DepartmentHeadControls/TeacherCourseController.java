...
public class TeacherCourseController {
    ...
    public ResponseEntity<String> deleteCourseFromClass(@RequestBody DeleteCourseDto deleteCourseDto) {
        try {
            teacherCourseServices.deleteCourseFromClass(deleteCourseDto);
            return ResponseEntity.ok("Course deleted successfully from the class");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("An error occurred: " + e.getMessage());
        }
    }
